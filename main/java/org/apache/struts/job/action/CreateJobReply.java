/**
 * User Create Job Action class: Once invoked lets User reply on an job opportunity 
 * 
...
public class CreateJobReply extends ActionSupport implements SessionAware{
    ...
    /**
	 * User are able to reply on an job opportunity
	 */
     ...
     public String execute() throws SQLException {
    	...
    	if (sessionmap.containsKey("email")&&(sessionmap.containsKey("public"))) {
    		...
    		if (getJrMessage().length() == 0) {  			
				msg = "Reply cannot be empty";
				return "JOB";
				}
    		
	    	int ctr = JobManager.createJobReply(jrMessage, jobID, userID);
